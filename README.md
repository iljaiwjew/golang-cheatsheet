...
6. If the base type is a struct type, the non-blank method names and field names must be distinct.(But promoted field can has the same name as well as method from embedded struct can have name that not promoted field alredy has)
