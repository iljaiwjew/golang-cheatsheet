...
4. The expression```pt.Mv``` is also legal because ```Mv``` is in method set of ```pt```(If pt was interface value, ```Mv``` would be not in method set and expression would be illegal). In this case the expression will be equivalent to ```(*t).Mv```.
